...
export default async function (m, conn = { user: {} }) {
  ...
  const lines = [
    `${chalk.cyan('❯')} ${chalk.white.bold('Bot:')} ${chalk.cyan((me || 'Bot') + (conn.user?.name ? ` ~${conn.user.name}` : '') + (conn.user?.jid == global.conn?.user?.jid ? '' : ' 【𝗦𝗨𝗕 𝗕𝗢𝗧】'))}`,
    `${chalk.cyan('❯')} ${chalk.white.bold('ID:')} ${chalk.yellow(senderJid || 'N/A')}`,
    `${chalk.cyan('❯')} ${chalk.white.bold('Usuario:')} ${chalk.yellow(sender || 'N/A')} / ${chalk.magentaBright.bold((user?.role && typeof user.role === 'string' ? user.role.replace(/\*/g, '') : '') || 'N/A')}`,
    `${chalk.cyan('❯')} ${chalk.white.bold('Premium:')} ${(user && typeof user.premiumTime === 'number' && user.premiumTime > 0) ? '✅' : '❌'}`,
    `${chalk.cyan('❯')} ${chalk.white.bold('Recursos:')} 🆙 ${(user?.level ?? 0)} ❇️ ${(user?.exp ?? 0)} 💎 ${(user?.limit ?? 0)} 🐱 ${(user?.money ?? user?.coin ?? 0)}`,
    (chatId || '').includes('@s.whatsapp.net') ? `${chalk.cyan('❯')} ${chalk.white.bold('Chat:')} ${chalk.greenBright('Privado')} ${chalk.dim(`con ${_name || 'Usuario'}`)}` : (chatId || '').includes('@g.us') ? `${chalk.cyan('❯')} ${chalk.white.bold('Chat:')} ${chalk.magentaBright('Grupo')} ${chalk.dim(`${chat || 'Grupo'}`)}` : (chatId || '').includes('@newsletter') ? `${chalk.cyan('❯')} ${chalk.white.bold('Chat:')} ${chalk.yellowBright('Canal')} ${chalk.dim(`${chat || 'Canal'}`)}` : '',
    ...
    `${chalk.cyan('❯')} ${chalk.white.bold('Tamaño:')} ${chalk.red((filesize || 0) + ' B')}`
  ].filter(Boolean)

  // Diagnóstico: si alguna línea queda exactamente 'null' o contiene ' null ' mostrará aviso una sola vez
  try {
    if (!global.__printedNullFlag) {
      const rawJoined = lines.join('\n')
      if (/^null$/m.test(rawJoined) || /\bnull\b/.test(rawJoined)) {
        global.__printedNullFlag = true
        console.log(chalk.bgRed.white('[DIAGNOSTICO] Se detectó la palabra "null" en el log. Reemplazando valores nulos por "N/A".'))
      }
    }
  } catch {}
